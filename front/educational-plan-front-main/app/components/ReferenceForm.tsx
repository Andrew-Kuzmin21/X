...
export const ReferenceForm = ({onClose}: { onClose: () => void }) => {
    ...
    const fetchReferenceData = async (referenceName: string) => {
        ...
        try {
            const response = await fetch(`http://localhost:8000${refConfig.path}/`);
    ...
    useEffect(() => {
        ...
        const fetchData = async () => {
            ...
            setError(null);
            try {
                const response = await fetch(`http://localhost:8000${selectedReference.path}/`);
                if (!response.ok) throw new Error(`Ошибка загрузки: ${response.statusText}`);
                const data = await response.json();
                ...
                if (data.length > 0) {
                    ...
                    const itemResponse = await fetch(
                        `http://localhost:8000${selectedReference.path}/${data[0].id}/`
                    );
    ...
    const handleItemClick = async (item: ReferenceItem | "add") => {
        ...
        try {
            const response = await fetch(
                `http://localhost:8000${selectedReference!.path}/${item.id}/`
            );
            if (!response.ok) {
    ...
    const handleSave = async () => {
        ...
        try {
            ...
            const url = selectedItem
                ? `http://localhost:8000${selectedReference.path}/${selectedItem.id}/`
                : `http://localhost:8000${selectedReference.path}/`;
            ...
            if (selectedItem) {
                ...
                const listResponse = await fetch(`http://localhost:8000${selectedReference.path}/`);
                if (listResponse.ok) {
        ...
        try {
            ...
            if (listResponse.ok) {
                ...
                if (updatedList.length > 0) {
                    ...
                    const itemResponse = await fetch(
                        `http://localhost:8000${selectedReference.path}/${updatedList[0].id}/`
                    );
                    if (itemResponse.ok) {
                        const itemData = await itemResponse.json();
                        setSelectedItem(itemData);
